...
At the module-level, files are prefixed with their problem number
(the canonical number if one exists, by order of introduction otherwise)
and optionally post-fixed by a salient non-redundant characteristics
(i.e., advertised difficulty level
if not otherwise evident from superordinate directory names).
